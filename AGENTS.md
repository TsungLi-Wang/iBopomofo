...
- New `project.pbxproj` file IDs for this fork: **下一個可用是 FACE0337+**（已用到 FACE0336，`ParticleRuleDisambiguator` + `particle-rules.tsv` + `confusion-alphas.tsv`）。加新檔前先 `grep -o "FACE0[0-9]\{3\}" McBopomofo.xcodeproj/project.pbxproj | sort -u | tail -1` 確認，撞號會讓專案檔開不起來。
### Swift & AppKit
