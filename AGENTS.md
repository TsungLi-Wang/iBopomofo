...
**Current line:** **i注音 / iBopomofo v2.9.4** (build 2298; tag `v2.9.4`). Stage-2 re-edit: soft-finalize (incl. Enter) keeps marked text; ←/→ then ↓ re-picks char right of cursor. Hard commit on focus loss / click-away. Shipping scores still λ/ν 0.75 → tw538 **387/537** (engine frozen). Handoff: `AI_HANDOFF_PROMPT.md` + `CHANGELOG.md`.
