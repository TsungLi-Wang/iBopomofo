...
**Current line:** **i注音 / iBopomofo v2.13.1** (build 2309; tag `v2.13.1`). Canonical UX: 定案 ≠ 送出; pause/。/，/Enter-while-underlined finalize without send (Standard layout 。=key `>` ，=key `<`); second Enter sends; post-commit ↓ = delete-and-recompose. Shipping scores λ/ν 0.75 → tw538 **387/537**. Handoff: `AI_HANDOFF_PROMPT.md` + `CHANGELOG.md`.
