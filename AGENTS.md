...
**Current line:** **i注音 / iBopomofo v2.9.5** (build 2299; tag `v2.9.5`). Enter = one-shot hard commit. Post-commit reselect: ←/→ then ↓ on committed text via surrounding-text replace (app-dependent). Pause/period/comma still soft-finalize while composing. Shipping scores λ/ν 0.75 → tw538 **387/537** (engine frozen). Handoff: `AI_HANDOFF_PROMPT.md` + `CHANGELOG.md`.
