...
**最後更新：2026-08-03**（**v2.9.0 三段式：自動定案 → 改字 → 回饋**；引擎凍結 tw538 387）

**先前：2026-07-27**：v2.8.0 公開 + 品牌 i注音/iBopomofo；全歷史機密掃描零洩漏。

**更早**：v2.7.0 版本可追溯正名（`549e4637`）；制度下沉；內部整頓；v2.7.0-dogfood Tab 預覽。
...
1. **發版**：**i注音（iBopomofo）v2.9.0**（build **2294**，tag **`v2.9.0`**）。三段式打字：停頓（800ms 恆開）／句號／Enter／逗號（後三可自訂）→ **同一套軟定案**（整句 rerank + 底線消失、仍 composing）→ 空白／↓ 手動改字 → `manual-correction.log` 回饋。引擎 walk/v2c **未改**；tw538 **387/537**。
2. **公開**：repo 公開開源（MIT + NOTICE）；gitleaks 全歷史零機密。品牌 i注音自 2.8.0。
3. **下一刀**：① Johnny dogfood：停頓定案、句號定案、定案後改字手感（含 800ms 是否要調）。② 手動改字 log 累積後再談重訓。③ λ/ν 是否改 0.70/0.50（391）仍為研究選項。
