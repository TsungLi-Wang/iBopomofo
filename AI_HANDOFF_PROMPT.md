...
**最後更新：2026-08-04**（**v2.9.1 停頓可自訂**；v2.9.0 三段式已發 Release）

**先前：2026-08-03**：v2.9.0 三段式自動定案（軟定案）→ 改字 → 回饋；引擎凍結 tw538 387。

**更早**：v2.8.0 公開 + 品牌 i注音；v2.7.0 版本可追溯。
...
1. **發版**：**i注音（iBopomofo）v2.9.1**（build **2295**，tag **`v2.9.1`**）。在 2.9.0 三段式之上，**停頓**改為可開關（預設開）+ 毫秒可自填（預設 800、下限 200），與 Enter／句號／逗號並列。引擎 walk/v2c **未改**；tw538 **387/537**。
2. **公開**：repo 公開開源（MIT + NOTICE）。Release：`v2.9.0` / `v2.9.1`（純 source，DMG 選配未附）。
3. **下一刀**：① Johnny dogfood 停頓關／開與改 ms。② 手動改字 log 累積後再談重訓。③ 若要 DMG 再補 upload。
