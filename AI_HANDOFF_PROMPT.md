...
**最後更新：2026-08-04**（**v2.9.3 選單「定案設定…」入口**；v2.9.2 Enter 一下到底）

**先前同日**：v2.9.2 Enter hard commit + 偏好定案分頁；v2.9.1 停頓可自訂；v2.9.0 三段式。
...
1. **發版**：**i注音（iBopomofo）v2.9.3**（build **2297**，tag **`v2.9.3`**）。選單有 **「定案設定…」** 直達偏好定案分頁。Enter 一下 hard commit；停頓／標點軟定案。引擎 **未改**；tw538 **387/537**。
2. **公開**：repo 公開開源（MIT + NOTICE）。Release：`v2.9.0`–`v2.9.3`（純 source）。
3. **下一刀**：① Johnny dogfood 定案設定入口 + Enter。② 手動改字 log 累積後再談重訓。
