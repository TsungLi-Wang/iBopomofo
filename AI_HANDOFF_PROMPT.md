...
**最後更新：2026-08-05**（**v2.13.1 句號／逗號觸發點生效**）

**先前**：v2.13.0 行為總則；v2.12.x 補指令作廢；v2.11.0 刪回重組。
...
1. **發版**：**i注音（iBopomofo）v2.13.1**（build **2309**，tag **`v2.13.1`**）。**句號／逗號觸發點生效**（標準注音 `>`／`<`）；與停頓同一 `hardCommitSentence`。引擎 **未改**；tw538 **387/537**。
2. **公開**：repo 公開開源。Release：`v2.9.0`–`v2.13.1`。
3. **下一刀**：① dogfood 勾句號／逗號打 。／，定案不送出；取消勾選只出標點。② AX 權限引導。③ 手動改字 log 重訓。
