...
1. **表徵改善是真的：把局部搭配明確編碼進去就有效。** I2（候選條件化的
   ...
   **0.792**、關鍵對 `作→作 vs 作→做` **0.764**，兩項都過事前門檻 0.75；
   I1 未過（0.768／0.738）。細節：
   `Source/Engine/eval/analysis/node-expert-collocation-representation.md`。
2. **但在凍結的 τ=0.5 上兩個都沒過 failure-mode gate**：
   I1 damage 15.3%→2.9% 卻把 `作→做` rescue 砍到 35%（又是「學會少改」）；
   I2 rescue 升（23→27、17→24）但 damage 也升（21→26），加權 net −2.03%。
   **不進正式 test。**
3. **關鍵發現：這是操作點問題，不是表徵問題。** 對齊出手量後，
   **I2 的決策曲線在每一個操作點都優於 R4**（出手 120 次時淨 +39 vs +24、
   傷害/出手 16.7% vs 24.2%）。I2 學到的是「更知道什麼時候該改」，
   不是「學會少改」。τ=0.5 是為舊表徵定的，新表徵在同一個 τ 落在更高出手量的
   位置 —— **剩下的是校準題，本棒明令凍結 τ，需要另外立案。**

## 前一棒（2026-08-18 · 棒⑭-H）

1. **判定：`GO TO REPRESENTATION EXPERIMENT`。** 資訊**在**上下文裡，
   是現在的表徵沒抽出來。同一份 544 節點診斷集上：R4 margin AUC **0.587**
   [0.530, 0.648]；一個最簡單的 bag-of-characters 線性分類器（同樣只用 ±6 的
   **walk 輸出**字，無洩漏）**0.728** [0.674, 0.780]；加邊界與引擎信心 **0.757**。
   **CI 不重疊。** 最關鍵的 `作→作 vs 作→做`：0.572 → 0.711 → 0.748。
   細節：`Source/Engine/eval/analysis/node-expert-representation-audit.md`。
2. **缺的不是更長的上下文，是「把局部搭配學起來」的能力。**
   加長窗口幾乎沒用（±6≈±10≈±15）；診斷 LR 學到的權重是詞彙搭配
   （手作 −0.58、大作 −0.53、神作 −0.33、合作 −0.33）。
   現在的 NodeExpert 看同一批 ±6 字，卻只用「字元 embedding 串接→淺層 MLP」。
   B（邊界）單獨 0.588、C（引擎信心）單獨 0.565，**都沒有足夠證據**。
3. **下一棒不要直接訓 production model。** 先設事前門檻（AUC 要到多少才動
   production）；上界（含洩漏的 gold ±15+B+C）也只有 0.807，**天花板存在**。
   途中我自己踩過一次洩漏（第一版 A 用了語料原文），已修正並保留為標記清楚的
   上界，差距約 +0.06 AUC。

## 前一棒（2026-08-18 · 棒⑭-G）

1. **針對性抑制也不行，但這次知道為什麼。** 只對 81 筆（train 0.09%）的子群
   加 margin penalty：`作→作` damage 15.3%→5.1%（G1）／0.7%（G2），
   **但 `作→做` rescue 被壓掉 70–78%（23→7／5）**。Gate A 過、**Gate B 沒過**。
   細節：`Source/Engine/eval/analysis/node-expert-targeted-suppression.md`。
2. **根因是可分性，不是配方**：`作→作·單` 與 `作→X·單` 的引擎側特徵完全相同
   （候選數 13、u(engine) −2.76、PMI 0/0、句長 7–8），R4 margin 的
   **AUC 只有 0.615**。模型能學到的最接近的東西就是「引擎說作又是單字時保守點」，
   而那涵蓋的正是 `作→做`／`作→座`。
   旁證：抑制沒有外溢到 engine=做（`做→坐` 8/4→7/4→4/3），所以遮罩是準的。
3. **這條線的極限已經明確**：切法問題解決了（⑭-F），資料量問題釐清了（⑭-E），
   剩下的是**表徵**——模型看得到的東西不足以分辨。要突破得動特徵集或架構，
   **超出前幾棒的凍結範圍，需要另外立案**。目前最好的是 G2：
   加權 net +0.33%、下界 −1.77%，仍未過凍結規則。**沒有 candidate 進正式 test。**
