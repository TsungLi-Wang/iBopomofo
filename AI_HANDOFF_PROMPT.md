...
**最後更新：2026-08-04**（**v2.9.2 Enter 一下到底 + 定案偏好分頁**）

**先前同日**：v2.9.1 停頓可自訂；v2.9.0 三段式自動定案。

**更早**：v2.8.0 公開 + 品牌 i注音；引擎凍結 tw538 387。
...
1. **發版**：**i注音（iBopomofo）v2.9.2**（build **2296**，tag **`v2.9.2`**）。**Enter 按一下**＝智慧選字＋hard commit（不再兩段）。停頓／句號／逗號仍軟定案。句子結束與手動改字 log 設定在偏好 **「定案」** 分頁；選單已清掉那一整排。引擎 walk/v2c **未改**；tw538 **387/537**。
2. **公開**：repo 公開開源（MIT + NOTICE）。Release：`v2.9.0`–`v2.9.2`（純 source，DMG 選配未附）。
3. **下一刀**：① Johnny dogfood Enter 一下／選單清爽／偏好定案分頁。② 手動改字 log 累積後再談重訓。
