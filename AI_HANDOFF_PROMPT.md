...
**最後更新：2026-08-05**（**v2.13.2 定案後 ←／→ 放行**）

**先前**：v2.13.1 句號／逗號觸發；v2.13.0 行為總則；v2.12.x 作廢。
...
1. **發版**：**i注音（iBopomofo）v2.13.2**（build **2310**，tag **`v2.13.2`**）。定案後 ←／→ **不攔截**（LINE／Telegram `selectedRange` 讀不到時全放行）；↓ 刪回重組維持。引擎 **未改**；tw538 **387/537**。
2. **公開**：repo 公開開源。Release：`v2.9.0`–`v2.13.2`。
3. **下一刀**：① dogfood LINE／Telegram 定案後方向鍵；TextEdit ↓ 重選。② AX 權限引導。③ 手動改字 log 重訓。
