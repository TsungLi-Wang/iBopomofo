...
**最後更新：2026-08-04**（**v2.9.4 Stage-2 定案後重選字**；Enter 軟定案）

**先前同日**：v2.9.3 選單「定案設定…」；v2.9.2 Enter 曾 hard commit（已反轉）；v2.9.1 停頓可自訂。
...
1. **發版**：**i注音（iBopomofo）v2.9.4**（build **2298**，tag **`v2.9.4`**）。Stage-2：定案後 ←／→／↓ 重選游標右方字；Enter＝軟定案（一下、底線消失、可編輯）。**LINE 等 Enter＝送出** 與可重選 **IMK 互斥**——真送出靠失焦／點別處。引擎 **未改**；tw538 **387/537**。
2. **公開**：repo 公開開源。Release：`v2.9.0`–`v2.9.4`（純 source）。
3. **下一刀**：① Johnny dogfood Stage-2 與聊天 app 送出體驗，必要時拍板取捨。② 手動改字 log 累積後重訓。
