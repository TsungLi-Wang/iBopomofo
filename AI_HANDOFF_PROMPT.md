...
**最後更新：2026-08-05**（**v2.13.0 行為總則：定案≠送出**）

**先前**：v2.12.x 補指令互相打架（已作廢）；v2.11.0 刪回重組；v2.10.x。
...
1. **發版**：**i注音（iBopomofo）v2.13.0**（build **2308**，tag **`v2.13.0`**）。**行為總則**：觸發點／有底線 Enter＝改字＋收底線、**不送出**；無底線 Enter＝送出；定案後 ↓＝刪回重組。引擎 **未改**；tw538 **387/537**。
2. **公開**：repo 公開開源。Release：`v2.9.0`–`v2.13.0`。
3. **下一刀**：① Johnny dogfood 四案（停頓定案不送出、定案後 ↓、再 Enter 送出、無觸發點兩下 Enter）。② AX 權限引導。③ 手動改字 log 重訓。
