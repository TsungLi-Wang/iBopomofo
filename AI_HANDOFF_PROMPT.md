...
3. **出貨前跑 `./scripts/ship-gate.sh`（預設只跑 CORE）**，`SHIP_GATE_STATUS=CORE` 或 `FULL` 才可出貨。
4. 難題考卷 EX1166 的分數只當參考，**不是出貨依據**。
5. **⛔ 禁止為 E2E／輸入法切換蠻幹**（2026-08-12 Johnny 明令）：
   - 關卡 1＋2＝離線，**不需要**選對輸入法。
   - 關卡 3＝可選 GUI 測試，預設**不跑**（`SHIP_GATE_E2E=0`）。
   - 選不到 i注音、出英文鍵序 → harness 環境問題；**一次失敗就停**，不准重試多輪「修好切換」。
