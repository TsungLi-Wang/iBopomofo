...
int main(int argc, char** argv) {
  if (argc < 6) {
    std::cerr << "usage: path_score_dump <sentences.jsonl> <data.txt> "
                 "<word-bigrams.tsv> <path-char-lstm.bin> <out.tsv> [nbest]\n";
    return 1;
  }
  ...
  long sid = 0, sents = 0, reproduced = 0, mismatch = 0, goldIn = 0;
  std::string line;
  while (std::getline(in, line)) {
    ...
    std::vector<double> rnns = scorer.scoreNBest(paths);

    size_t bestIdx = 0;
    double bestFinal = -std::numeric_limits<double>::infinity();
    for (size_t pi = 0; pi < nbest.size(); ++pi) {
      double fused = nbest[pi].walkScore + kNu * rnns[pi];
      if (fused > bestFinal) { bestFinal = fused; bestIdx = pi; }
    }
    std::string repro;
    for (const auto& wd : nbest[bestIdx].words) repro += wd;
    if (repro == walkOut) ++reproduced; else ++mismatch;

    bool anyGold = false;
    for (size_t pi = 0; pi < nbest.size(); ++pi) {
      const auto& rp = nbest[pi];
      std::string joined;
      for (const auto& wd : rp.words) joined += wd;
      std::vector<std::string> pc = utf8Chars(joined);
      long e = 0;
      if (pc.size() != goldChars.size()) {
        e = static_cast<long>(goldChars.size());
      } else {
        for (size_t i2 = 0; i2 < goldChars.size(); ++i2) {
          if (pc[i2] != goldChars[i2]) ++e;
        }
      }
      if (e == 0) anyGold = true;
      double uni = 0.0;
      for (size_t ni = 0; ni < rp.nodes.size(); ++ni) {
        ...
        if (ni < rp.selectedUnigramIndices.size() &&
            rp.selectedUnigramIndices[ni] < ug.size()) {
          uni += ug[rp.selectedUnigramIndices[ni]].score();
        }
      }
      out << sid << "\t" << pi << "\t" << e << "\t"
  ...
  std::cerr << "ERROR_SENTENCES " << sents << "\nFUSED_ARGMAX_REPRODUCES_WALK "
            << reproduced << "\nMISMATCH " << mismatch
            << "\nGOLD_PATH_IN_NBEST " << goldIn << "\n";
  return 0;
}
