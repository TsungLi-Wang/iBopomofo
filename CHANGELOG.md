...
- **自動標音取代人工貼小麥**：`auto_annotate.py` 用引擎詞庫最長匹配標音。
  ...
  人工那一步因此拿掉。
- **同音頻率先驗壓縮（路線 A）已實作但未證實、預設關閉**：
  `ReadingGrid::setConfusionAlphas` + `Source/Data/confusion-alphas.tsv`。
  train +3.4 個百分點，**但封存集只有 +1.0%，落在 ±3% 的雜訊裡**。
  沒有表時行為完全不變。詳見 `AI_HANDOFF_PROMPT.md`。
