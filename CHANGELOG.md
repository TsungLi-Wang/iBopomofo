...
### 內部（2026-08-24）：的／得規則可從輸入法選單開關

比照「神經路徑重排」。預設開（與出貨一致）。關掉只跳過 `rescoreWalk`，不卸載規則表。
消融：`SHIP_GATE_ABLATION=1 ./scripts/ship-gate.sh` 印四種組合（LSTM × 的/得，Contextual 皆開）相對「底層硬扛」的救／壞／淨；不改 CORE 出貨判定。

### 內部（2026-08-24）：UOM 改 log10、μ 4.0→1.5（與 PMI 同量級）

**沒有改 λ、沒有對 OOV 加負分。** 顧問另外兩點（找不到 pair 罰 −0.5、λ 0.75→0.6）互斥且未在真實語料上量過，未做。

- `userScore`：`ln(1+count)` → `log10(1+count)`，與詞庫 unigram／PMI 同一底。
- `kDefaultMuUser`：4.0 → 1.5。兩次校正的 DP 加分從 ≈+4.39 變成 ≈+0.72，不再輾壓中位 λ·PMI（≈+1.07）。
- 合成測試：C_min 只要求翻過 0.50 的近距同音；3.0 的頻率差距不再被兩次校正翻掉。

### 內部（2026-08-24）：`algorithm.md` 與 `reading_grid.h` 對齊現役 walk

顧問讀到「目前僅使用 Unigram」＋已不存在的 `Relax()`／`TopologicalSort()`，把出貨架構誤判成「純 unigram DAG、外面四層補丁」。檔頭警告本身也把 word-bigram 列成「沒寫到的四層」之一，加重誤讀。已改寫 walk 段、關鍵路徑表、以及 `reading_grid.h` 的過時註解：詞 bigram + UOM 在 `walk()` 邊權裡。

### 內部（2026-08-24）：句首學過「裡」不再把「理解」拆成「裡解」

**使用者可見**：打「理解」兩個音，畫面應出「理解」，不必先手選一次。

- **根因**：句首把「理」改成「裡」滿兩次後，UOM 給所有句首 ㄌㄧˇ 的「裡」加分（μ·log(1+2)≈+4.4），大於詞典詞「理解」對兩顆單字的優勢，walk 走出「裡＋解」。↓ 清單第一名仍是「理解」（按詞長排），看起來像引擎知道卻不用。
- **修法**：walk 之後，連續單字節點若正好覆蓋詞圖上已有的多字詞，改走那個詞。這次組字裡手選過的單字不黏回去。
- **不改**：哪裡＋解（「哪裡」已是詞，不會拆去湊「理解」）。
...
**第二次量測（13:56，同棒重跑）**：census 從 1 → **13 筆真實定案**
（33 節點 / 38 字 / 0 手選，跨度 50 分鐘，同一 build、同一 IME 進程）——
**持續性已證實**。污染檢查再驗一次，SHA 仍未變。

但同時暴露一個第一次看不出的缺口：**correction v2 真實事件仍是 0 筆**，
`manual-correction.log` 自測試套件那次之後一行都沒再增加。
13 次定案 0 次手選有兩種解釋（真的沒修正 / 分子路徑沒動），目前分不出來。
依 v1 歷史速率約 1.7 筆/小時，50 分鐘觀察到 0 筆在統計上正常（P≈25%），
...
sha256 與進場相同）。新增的 `NodeHomophoneExpert` **預設完全不掛**——
不設 `IBOPOMOFO_NODE_EXPERT` 環境變數時行為逐位相同，也還沒接進 app。

- **`docs/decisions/0008`**：`0007` 那個位置第一次真的開做。預測單位是 walk 節點
  （不是字窗、不是下一個字），只在節點既有候選裡改選，預設棄權，只開「作做坐座」。
- **結果 NO-GO**：兩份真實語料 自然 救10壞38（淨 −28，p=9.7e-05）、X 救1壞6。
  權重留研究目錄，不合進 master。
- **圍堵有效，這是與棒⑫ 的關鍵差別**：自然驗證集只改 62 句（棒⑫ 3,501）、
  目標外 5 句（棒⑫ 3,409）、整句逐字正確率 95.773%→95.731%（棒⑫ 掉到 89.58%）、
  延遲 +0.15ms/句（棒⑫ 133ms/句）。其他五組 A/B 全部 0 救 0 壞。
- **安全契約釘在 gtest（9 個全綠）**：白名單、ㄉㄜ˙ 硬擋、棄權、不蓋手選、
  沒 context model 就不動、沒載模型＝惰性、拒收 `LWLSTM` magic。
- **新工具**：`scripts/node-expert-parity.sh`（C++ 與 PyTorch 逐題同分，換模型第一道關卡）、
  `scripts/node-expert-ab.sh`（專家關 vs 開；**不是** `model-ab.sh` 那把尺）、
  `node_expert_collateral.py`（誤傷＋整句逐字正確率）、
  `node_sample_extract`（真的跑 walk 抽節點樣本）。
- **三條新死路**進 `docs/dead-ends.md`：節點層專家對作做坐座；難例只按「引擎選錯」挑
  而不按方向配平；拿未稽核的 PTT 原文當 dev 定門檻。

### 內部（棒⑪ · 2026-08-13）：把尺補齊、量地圖。**零使用者可見行為、未發版**

引擎、規則表、詞庫、神經權重**一律未動**（`path-char-lstm.bin` 收工 sha256
與進場相同）。這一棒補的是「怎麼判斷下一刀該砍哪裡」的工具與地圖。

- **`docs/decisions/0007`**：神經消歧專家只能接**節點的候選清單**，
  掛點沿用 `selectOverrideUnigram`；棄權＝不 override；使用者手選永不被蓋。
  **只定接線位置，不開做。**
- **`scripts/model-ab.sh`**：換神經權重的判準。ship-gate 比的是「規則開／關」，
  兩邊同一個模型 —— 換權重它抓不到（棒⑩ 那個淨傷害 −36 的模型照樣跑出 CORE）。
  統計交給既有的 `compare_dumps.py`，不重寫檢定。
  `--self`（同權重跑兩次）在 sample 與**兩份真實語料**上都是 **救 0／壞 0**，
  評分流程是決定性的。AGENTS 補了「改了什麼 → 用哪把尺」對照表。
- **`scripts/correction-census.sh`**：手動校正 log 的彙總普查（只印次數與字對，
  **永不印句子／前後文／時間戳**）。現況：schema v1 有 310 行，但其中
  **286 行 `wrong_char` 是空的**，可成對的只有 24 筆，扣掉「選回同一個字」
  13 筆之後，**真正換了字的校正事件只有 11 筆**。
  → 判斷：**太少，免談群眾層**（`docs/decisions/0003` 那條路的第一個數字）。
  （`wrong_char` 空白**不是缺口**：組字中手選走 `KeyHandler.mm:379` 一律寫空字串，
  只有定案後 ↓ 重選才會填（`InputMethodController+ShadowReselect.swift:222`）。
  所以「真正換了字的 11 筆」＝定案後重選的次數，這正是 0003 要的那種訊號，
  只是量太少。）
- **`Source/Engine/eval/analysis/real-corpus-error-layers.md`**：
  第一次在**兩份真實語料**上算 oracle 上界與錯誤分層（0004 那張是 EX1166 封存集，
  未覆寫）。最大宗仍是**整句解碼錯**（自然 47.8%、X 36.9%），
  且比考卷上更大宗；但「的／得」在真實語料上根本不是瓶頸（現況 96.4%／98.6%），
  最弱的是「作做坐座」（84.9%／82.2%，O1 93.5% vs O2 98.6% ——
  節點層才有空間，正是 0007 的論據）。

### 內部（棒⑩ · 2026-08-13）：作做坐座 v2d 實測 NO-GO，權重未動

issue #9 要把 v2d 的做法套到其他同音字組。這一棒試了**作做坐座**一組，
**結果是淨傷害，權重已還原，出貨模型一位元組未變。**

量測方法：同一份出貨配置、只換 `path-char-lstm.bin`，兩份真實語料逐題配對比較
（McNemar 精確檢定）。**不是**用 `ship-gate.sh` —— 它比的是「規則開 vs 規則關」，
兩邊同一個模型，對換權重不敏感（那個淨傷害的模型照樣跑出 `SHIP_GATE_STATUS=CORE`）。

| | 救 | 壞 | 淨 | p |
|---|---|---|---|---|
| 作做坐座 · 自然驗證集(PTT) | 20 | 56 | −36 | 4.4e-05 |
| 作做坐座 · X驗證集 | 2 | 8 | −6 | 0.109 |

兩份同方向，PTT 那份顯著。訓練當下語料 dev 就已經 82.4% → 81.5% 變差。
原因先驗上就知道：這組路徑層天花板只有 71.1%，最大宗的錯是「上下文不足」與
「整句解碼」，對比訓練打的是第三大宗。已寫進 `docs/dead-ends.md`。

順帶修正一個會誤導下一棒的程序問題：新增同音組要從
`path-char-lstm-spoken-v2d.bin` 起訓，不是 v2c —— 從 v2c 起訓會把在／再的微調丟掉。

### 修復（棒⑩ · 2026-08-13）：拆詞校正現在記得住了（issue #10）

**人話**：打「你先ㄗㄨㄛˋ」出「你先做」，你開候選窗改成「坐」；下次同樣前文再打一次，
它以前還是會給你「做」—— **改了等於白改**。現在會記得。

只影響「手選把既有的詞拆開」這一類（先做 → 先＋坐）。單字換單字的校正走另一條路，
行為完全不變。

**根因**：`observe()` 在拆詞（`breakingUp`）時用**校正後**的 walk 組鍵，
鍵裡是「坐」；而 `suggest()` 用**當下**的 walk 組鍵，當下 walk 仍走「先做」——
兩把鍵永遠對不上，等於「只有在已經改對的狀態下才查得中」。
不是棒⑥ 改名造成的，是 upstream 就有的語意。

**修法**：拆詞時**額外**記一把「拆開前」的鍵（就是 `suggest()` 下次會組出來的那把）。
純加法：原本那把鍵的意義不變，既有 `user-override-cache.dat` 的條目照舊可用。
舊 cache 沒有新鍵，那些舊條目會跟現在一樣繼續 miss，升級後第一次校正就會寫進新鍵。

**怎麼驗的**：`Source/Engine/UserOverrideModelTest.cpp` 新增三個 GoogleTest。
量的是使用者看得到的需求（「改對一次，同前文重打要記得」），
**不是**去斷言 `observe()` 自己組出來的鍵長什麼樣 —— 那等於拿機制自己當驗收。
含一個 fixture 前提檢查（確認真的重現了「詞路蓋過單字」）與一個對照組
（單字對單字校正不得被改壞）。修改前紅、修改後綠；`ctest` 157/157 全過。
