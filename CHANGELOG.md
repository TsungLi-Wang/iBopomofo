...
### 內部（棒⑫ · 2026-08-14）：訓了一顆注音條件的節點層模型，**NO-GO**。零使用者可見行為、未發版

引擎行為、規則表、詞庫、出貨神經權重**一律未動**
（`path-char-lstm.bin` 收工 sha256 與進場相同；新的節點層打分器
**預設完全不掛**，不設 `IBOPOMOFO_NODE_SCORER` 環境變數時行為逐位相同）。

- **`docs/decisions/0008`**：規格與完整數字。職位＝「這個音在這句裡該出哪個候選」
  （P(字|注音,左右文)，留詞圖、換打分器），不走整句翻譯式 P2C。
  模型 15.4M 參數／61.6MB（1GB 上限的 6%）。
- **結果 NO-GO**：同分布 dev 上目標字淨 +22，兩份真實語料卻是
  **自然 −28、X −14**，整句逐字正確率 95.8%→89.6%／97.3%→91.8%。
  權重留在研究目錄，出貨檔一行未動。
- **新工具（以後換模型都要用）**：`scripts/node-scorer-parity.sh`
  （C++ 與 PyTorch 逐題同分，換模型的第一道關卡）、
  `scripts/node-scorer-ab.sh`（掛 vs 不掛的逐題配對）、
  `node_scorer_collateral.py`（**整句逐字正確率** —— 現有三支尺只看目標那一個字，
  漏掉的正是這次最大宗的傷害）。
- **三條新死路**已進 `docs/dead-ends.md`：固定門檻硬蓋 walk；
  只把髒標籤排除在 gold 之外不等於排除污染；只看目標字會漏掉一半傷害。
